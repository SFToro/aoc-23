...
fn main() {
    ...
    // println!("{:?}", my_hash_map);
    let root = my_hash_map.clone().into_values().max().unwrap();
    println!("Ocupado {}", root);
    println!("Libre {}", 70000000 - root);
    println!("Necesitamos {}", 30000000 - (70000000 - root));
    println!(
        "{:?}",
        my_hash_map
            .into_values()
            // .filter(|v| *v >= (23909866))
            .filter(|v| *v >= (30000000 - (70000000 - root)))
            .min()
            .unwrap()
    );
}
